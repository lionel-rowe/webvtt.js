...
import { WebVttParser } from "../parser.mjs";
import { WebVttSerializer } from "../parser.mjs";
const entities = JSON.parse(readFileSync("./html-entities.json"));
...
describe("Tests the file parser", () => {
  ...
  for(let path of files.filter(p => p.match(/\.vtt$/)))  {
    ...
    const vtt = readFileSync(dir + path, 'utf-8');
    it("matches the expected assertions for " + path, () => {
      const js = readFileSync(dir + path.replace(/\.vtt$/, '.js'), 'utf-8');
...
describe("Tests the cue parser", () => {
  ...
  for(let path of files.filter(p => p.match(/\.vtt$/)))  {
    ...
    it("matches the expected DOM tree for " + path, () => {
      const expected = JSON.parse(readFileSync(dir + path.replace(/\.vtt/, '.json'), 'utf-8'));
      const res = parser.parse(vtt);
